...
def doctor_view_patient_view(request):
    ...
    # for profile picture of doctor in sidebar
    doctor = models.Doctor.objects.get(user_id=request.user.id)
    test_results = models.Testresult.objects.all()
    sruvey_results = models.Surveyresult.objects.all()
    hospital_p = models.HospitalPatient.objects.all().filter(
        status=True, assigneddoctorid=request.user.id)
    if request.method == 'POST':
        if request.POST["action"] == "get_test_sreen":
            name = request.POST.get('name')
            print("hello")

    return render(request, 'hospital/doctor_view_patient.html', {'patients': patients, 'doctor': doctor, 'survey_results': sruvey_results, 'test_results': test_results, 'hospital_p': hospital_p})
...
def test_results(request):
    if request.method == 'POST':
        ...
        return render(request, 'hospital/test_results.html', {'name': name})
    else:
        return render(request, 'hospital/test_results.html')
